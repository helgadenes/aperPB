...
"""
This script combines scan2fits_spec.py and scan2fits_spec.py to speed up the ~40 minute part of reading in the csv file. 
The script creates fits cubes from drift scan data, using the previously created .csv files. 
Two sets of maps are created, one averaged for all of the antennas and one set per each antenna. 
...
def main():
	...
	print("Making beam maps per antenna: ")
	for ant in range(12):
		...
		for beam in beams:
			...
			for f in range(freqchunks):
				...
				# Find the reference pixel at the apparent coordinates of the calibrator
				ref_pixy = (calibnow.dec.deg - max(y)) / cell_size + 1      # FITS indexed from 1
				ref_pixx = (-min(x)) / cell_size + 1                        # FITS indexed from 1
				ref_pixz = 1                                                # FITS indexed from 1
